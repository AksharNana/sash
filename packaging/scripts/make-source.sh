...
#
# Produces OUTDIR/sash_<version>.orig.tar.gz (shared, byte-identical for all
# series) and, per series, sash_<version>-<rev>~<series>1_source.changes.
#
...
orig=sash_$version.orig.tar.gz
if [ ! -f "$outdir/$orig" ]; then
    stage_tree "$work/sash-$version"
    tar --sort=name --owner=0 --group=0 --numeric-owner \
        --mtime="@$(git -C "$root" log -1 --format=%ct)" \
        -C "$work" -cf - "sash-$version" | gzip -n > "$outdir/$orig"
fi

for series in "$@"; do
    dir=$work/build-$series/sash-$version
    mkdir -p "$dir"
    tar -xzf "$outdir/$orig" -C "$work/build-$series"
    cp "$outdir/$orig" "$work/build-$series/$orig"
    cp -R "$root/packaging/debian" "$dir/debian"

    V="$version-$revision~${series}1" S=$series D=$(date -R) perl -pi -e '
        s/^sash \(.*?\) .*?;/sash ($ENV{V}) $ENV{S};/ if $. == 1;
        s/^( -- .*?  ).*$/$1$ENV{D}/ if /^ -- / && !$done++;
    ' "$dir/debian/changelog"

    (cd "$dir" && dpkg-buildpackage -S -sa -d -us -uc)
    mv "$work/build-$series"/sash_*"~${series}1"* "$outdir/"
done
