...
#
# Usage:
#   scripts/asash-docker.sh [ASASH_OPTIONS] FILE
#
# It works with either Docker or Podman, auto-detecting whichever is installed
# (preferring Docker if both are present).
#
# Environment:
#   ASASH_IMAGE        Image (tag) to run (default: sash)
#   ASASH_RUNTIME      Container runtime to use (default: auto-detect docker/podman)
#   ASASH_DOCKER_ARGS  Extra args for `<runtime> run`, e.g. "--privileged" for CRIU
#
# Tip: alias it for convenience, e.g.
#   alias asash='/path/to/scripts/asash-docker.sh'

set -eo pipefail

image="${ASASH_IMAGE:-sash}"

# Pick a container runtime: an explicit ASASH_RUNTIME wins, otherwise prefer
...
# shellcheck disable=SC2206 # intentional word-splitting of ASASH_DOCKER_ARGS
extra_args=(${ASASH_DOCKER_ARGS:-})

exec "$runtime" run --rm "${extra_args[@]}" "${docker_args[@]}" "$image" "${asash_args[@]}"
